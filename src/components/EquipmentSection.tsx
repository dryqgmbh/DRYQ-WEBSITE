      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-10 mb-16 items-end">
          ...
          <div className="flex flex-wrap gap-2.5 lg:justify-end reveal">
            {['Eigene Technik', 'Passende Geräteleistung', 'Saubere Aufstellung', 'Dokumentierter Einsatz'].map((t) => (
              <span key={t} className="text-xs font-semibold bg-white/6 border border-white/10 text-gray-300 px-3 py-1.5 rounded-full">{t}</span>
            ))}
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {equipment.map(({ Icon, id, title, sub, desc, spec }) => (
            <div key={id} className="group bg-white/4 hover:bg-white/7 border border-white/8 hover:border-[#35D04F]/30 rounded-2xl p-6 transition-all duration-300 reveal">
              {/* Image slot – replace with: <Image src={`/assets/dryqtech-${id}.png`} alt={title} ... /> */}
              <div className="aspect-video bg-[#1A1F26] rounded-xl mb-5 flex flex-col items-center justify-center border border-white/6">
              ...
              <div className="flex items-start justify-between gap-2 mb-1">
                ...
                <span className="flex-shrink-0 text-[10px] font-semibold bg-[#35D04F]/15 text-[#35D04F] px-2 py-0.5 rounded-full">{spec}</span>
              </div>
